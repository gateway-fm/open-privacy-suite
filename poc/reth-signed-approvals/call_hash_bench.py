#!/usr/bin/env python3
"""Paired stock/strict/call-only measurements inside real Reth payload construction.
...
def benchmark(samples=9, batch=32, profile=False):
    ...
    for workload, count in [("single_storage_write", 0), ("three_calls", 1), ("eighteen_calls", 16)]:
        for pair in range(3):
            ...
            for mode in modes[pair:] + modes[:pair]:
                os.environ["OPS_FINGERPRINT_MODE"] = "direct"
                with r.node(f"compare-{workload}-{pair}-{mode}", disabled=mode == "baseline") as n, contextlib.closing(r.Client(n, connect=mode != "baseline", hash_mode="strict" if mode=="strict" else "calls")) as c:
    ...
    for workload in sorted({x["workload"] for x in rows}):
        ...
        summary.append({"workload": workload, "median_us_per_tx": medians,
                        ...
                        "extra_us_per_tx": medians["calls"] - medians["baseline"],
                        "calls_time_increase_vs_baseline_percent": (medians["calls"] / medians["baseline"] - 1) * 100,
                        "calls_time_reduction_vs_strict_percent": (1 - medians["calls"] / medians["strict"]) * 100,
                        "stage_mean_us_per_tx": stage_us,
                        "stage_median_us_per_tx": stage_median_us})
    report = {"method": "Same binary; module off, strict V2, calls V3. Approval batch size 32; all modes execute the same two preflight RPCs per transaction. Internal default_ethereum_payload time including execution and roots, divided by included tx count. Identical preflight warming. Rotated mode order; one discarded warmup per node. All three block hashes equal for every sample. RPC, preflight, approval signature verification/delivery, wait, and engine import excluded.",
