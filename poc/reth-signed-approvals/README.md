...
В сценариях `node` тестовый клиент [client/main.go](client/main.go) использует настоящий `nodeapproval.New` и `Enqueue`: OPS выполняет preflight, подпись, согласование TTL, доставку, повторы и хранение batch. Проверка restart сохраняет процесс отправителя и адрес producer: после смены boot id его lane сама повторно доставляет разрешение, без нового preflight, ручного `Deliver` или повторной отправки транзакции. Прямые gRPC-вызовы оставлены только для намеренно неверных подписей, формата и граничных случаев срока действия. Для воспроизводимого построения payload и измерения producer тестовая фикстура ждёт подтверждения через счётчик отправителя; сам `Enqueue` по-прежнему не ждёт доставки.

[Проверка исправлений 25 сентября 2026: 23 базовых сценария, 19 сценариев calls V3, интеграция настоящего OPS с PostgreSQL и два Go regression-теста под race detector](evidence/review-followups-2026-09-25/tests.json). [Лог живого отправителя при restart Reth](evidence/review-followups-2026-09-25/restart-recovery-sender.log) показывает смену boot id, автоматическую повторную доставку и два подключения одного процесса OPS.
