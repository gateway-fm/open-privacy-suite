...
The RPC endpoint shown in the UI still reads `localhost`. Set
`VITE_BACKEND_URL=http://YOUR_LAN_IP:8080` if you need it to display the LAN
address.

### Wallet login from a phone

The Privado ID wallet calls back to the **API** (port 8080), not the app, so
8080 must be reachable too — `HOST_BIND=0.0.0.0` covers that on the same wifi.
If the wallet will not accept a plain-http callback, tunnel the API and point
`BASE_URL` at the tunnel:

```bash
ngrok http 8080
BASE_URL=https://YOUR-TUNNEL.ngrok-free.app HOST_BIND=0.0.0.0 make run
```

Tunnel the API, not the app — tunnelling 5173 alone breaks the callback.
