...
<EnvTable vars={[
  ...
  { name: "OPS_APPROVAL_PREFLIGHT_RATE", default: "20", description: "Applies only when signed transaction approvals are enabled (OPS_APPROVAL_TARGETS set). How many transactions per second each identity may have simulated on the node for approval, sustained; decimals allowed, 0.001 to 100000. The budget is shared by all proxy instances through Redis when REDIS_URL is set, and held per instance otherwise (and per instance while Redis is unreachable). A transaction over the budget is refused with 429 before anything is simulated. An invalid or out-of-range value stops start-up. Environment only: not read from CONFIG_FILE." },
  { name: "OPS_APPROVAL_PREFLIGHT_BURST", default: "40", description: "How many simulations an identity may run back to back after being idle before OPS_APPROVAL_PREFLIGHT_RATE applies; a whole number, 1 to 100000. An invalid or out-of-range value stops start-up. Environment only: not read from CONFIG_FILE." },
]} />
