...
In the dashboard the Ban control on your own row is inert and explains why on hover, both in the user list and on the user detail page. As always the server-side check is the boundary; the UI only avoids offering an action that would fail.
