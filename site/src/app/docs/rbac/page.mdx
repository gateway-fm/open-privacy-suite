...
## Cross-Organization Authorization Oracle

`POST /api/v1/admin/cross-org-authorization-oracle` lets an explicitly trusted service test a JSON-RPC operation before submission.
It is disabled by default. When enabled, use the dedicated `CROSS_ORG_AUTHORIZATION_ORACLE_TOKEN` in the
`X-Cross-Org-Authorization-Oracle-Token` header. Admin, operator, and JWT credentials cannot use this endpoint.

The request identifies one subject by DID or linked Ethereum address. An optional `org_id` selects the organization.
A supplied `from` address must link to the subject. In `verdict_only` mode the response contains only `allowed` and a safe
reason category; `full_simulation` additionally returns authorized trace artifacts.
...
Trace checks use the same upstream RPC credential and configured shared budget as live calls; timeout and concurrency limits also protect resources.
