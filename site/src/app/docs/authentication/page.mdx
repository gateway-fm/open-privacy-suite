...
| `network_not_supported` | This deployment has no state resolver for the wallet's identity network, so the proof cannot be verified here. | Nothing — retrying cannot help. The operator must enable that network, or the user must sign in with a wallet on a supported one. |
...
`network_not_supported` is reported rather than collapsed because it discloses nothing that is not already public: the set of supported networks is published by `GET /api/v1/auth/providers`, which takes no authentication, and the code itself names no network. Withholding it only cost the person at the browser the one piece of information that would have told them the deployment — not their wallet — needs attention.
